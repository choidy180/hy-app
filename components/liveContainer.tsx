...
const LiveContainer = () => {
    ...
    useEffect(()=>{
        setInterval(() => {
            setRealTime(moment().format('LTS'));
        }, 1000);
    },[])
    return (
        <div className={`absolute left-[20px] top-[20px] w-[300px] p-[20px] bg-[#f1f2f6] shadow-xl rounded-[14px] cursor-pointer transition-all hover:scale-[1.02] ${realTime === '' && '!hidden'}`} onClick={()=> setTodayLive(!todayLive)}>
            ...
            <div className="font-semibold text-base flex justify-start items-center mt-[10px]">
                ...
                <h4 className='text-[24px] pl-[6px]'>{todayLive ? '오후 9시에 만나요!' : '오늘은 휴방입니다!'}</h4>
            </div>
